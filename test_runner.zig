...
//!
//! Vendored copy of Zig 0.16.0's `lib/zig/compiler/test_runner.zig` with a
//! single patch (search "PATCH (gvm)") that works around a bug in that release:
//! the stock runner's `--fuzz` code path fails to compile because it passes a
//! `*builtin.StackTrace` to `std.debug.writeStackTrace`, which now expects a
//! `*const debug.StackTrace`. Without this, `zig build fuzz --fuzz` cannot
//! build the instrumented test binary. Delete this file and drop the
//! `.test_runner` wiring in build.zig once the toolchain ships the fix.
        ...
        fn test_one() callconv(.c) bool {
            ...
            testOne(ctx, @constCast(&testing.Smith{ .in = null })) catch |err| switch (err) {
                error.SkipZigTest => return true,
                    ...
                    // PATCH (gvm): Zig 0.16.0's stock runner calls
                    // std.debug.writeStackTrace(trace, ...) here, but `trace` is a
                    // `*builtin.StackTrace` while the fn now wants `*const debug.StackTrace`,
                    // so the stock runner fails to compile under `-ffuzz`. Drop the trace
                    // print (cosmetic only) so the built-in fuzzer builds. See build.zig.
                    const stderr = std.debug.lockStderr(&.{}).terminal();
                    stderr.writer.print("fuzz test failed with error.{t}\n", .{err}) catch {};
                    std.process.exit(1);
                },
            };
            if (log_err_count != 0) {
